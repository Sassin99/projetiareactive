...
function preload() {
  ...
  vaisseau = loadImage('./assets/vehicule.png');
  obstacle = loadImage('assets/planetobstacle.avif');
  enemiImg = loadImage('./assets/image.png'); // Correction ici
}
...
function draw() {
  ...
  vehicules.forEach((v, index) => {
    ...
    if (m == "s") {
      ...
      v.applyBehaviors(enemi.pos.copy(), obstacles, vehicules);
      v.applyForce(v.seek(enemi.pos.copy()));
    }
...
function keyPressed() {
  ...
  if (key == "v") {
    vehicules.push(new Vehicle(random(width), random(height), vaisseau));
  }
  if (key == "d") {
    ...
    m = "e";
    enemi = new Enemi(random(width), random(height), random(20, 50), enemiImg); // Correction ici
  }
}
