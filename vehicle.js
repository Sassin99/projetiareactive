...
class Vehicle {
  ...
  seek(target, arrival = false) {
    ...
    if (arrival) {
      ...
      if (distance < slowRadius) {
        desiredSpeed = map(distance, 80, slowRadius, 0, this.maxSpeed);
      }
    }
  ...
  */
  ...
  drawVehicle() {
    push();
    // Déplacement vers la position du véhicule
    translate(this.pos.x, this.pos.y);
    // Rotation selon la direction du mouvement
    rotate(this.vel.heading() - PI / 2);

    // Si une image est fournie, on l'utilise
    if (this.image) {
        imageMode(CENTER);
        // Ajuster la taille en changeant les coefficients ici
        image(this.image, 0, 0, this.r_pourDessin * 5, this.r_pourDessin * 5); // Taille ajustée
    } else {
        // Si aucune image, dessiner un triangle comme alternative
        // stroke(255);
        // strokeWeight(2);
        fill("red");
        circle(0, 0, 16);
    ...
    // Dessiner un cercle pour le débogage si activé
    if (Vehicle.debug) {
        noFill();
        stroke(255);
        circle(0, 0, this.r); // Cercle de rayon `r` autour du véhicule
    }
    pop();

    // Vecteur de vitesse pour le débogage
    if (Vehicle.debug) {
